...
from transformers import (
    ...
    AutoModelWithLMHead, AutoModelForCausalLM, AutoTokenizer
)
...
# read in csv and set character name
data = pd.read_csv('loki_transcripts.csv',
...
df = df.dropna()  # drop any None rows so we do not get TextEncodeInput error
...
class Args():
    def __init__(self):
        self.output_dir = 'output-mediuim'
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    args.train_batch_size = 4 # args.per_gpu_train_batch_size * max(1, args.n_gpu)
...
# Load trained model
# tokenizer = AutoTokenizer.from_pretrained('microsoft/DialoGPT-medium')
# model = AutoModelWithLMHead.from_pretrained('output-mediuim')
...
#     print("LokiBot: {}".format(tokenizer.decode(chat_history_ids[:, bot_input_ids.shape[-1]:][0], skip_special_tokens=True)))
